...
mandarin <- mandarin %>%
  mutate(
    the_freq = log(the_freq),
    aan_freq = log(aan_freq),
    demo_freq = log(demo_freq),
    PRP_freq = log(PRP_freq), 
    other_det_freq = log(other_det_freq), 
    one_freq = log(one_freq)
  )
spanish <- spanish %>%
  mutate(
    the_freq = log(the_freq),
...
mandarin$ratio <- mandarin$demo_freq / mandarin$the_freq
summary(mandarin$ratio)
mandarin1 <- filter(mandarin1, log(num_uniq_word) > 3)
summary(lm(mandarin1$ratio ~ mandarin1$num_uniq_word))
ggplot(mandarin1, aes(x=num_uniq_word, y=ratio)) + geom_point() + stat_smooth(method='lm')
```